...
pragma solidity 0.8.17;
/// @title A helper contract for royalty config 
contract royaltyConfigHelper is Ownable{

    error checkRateError();
    error checkLengthError();
    ...
    mapping (address => RoyaltyConfig) internal royaltySetting;
    
    
    function setRoyaltyConfig(address _tokenAddress, uint256[] memory _royaltyRate, address[] memory _royaltyAddress) public onlyOwner {

        uint256 checkRate = 0;
        for (uint256 index = 0; index < _royaltyRate.length; index++) {
            checkRate += _royaltyRate[index];
        }

        if(checkRate > 50) revert checkRateError();

        if(_royaltyAddress.length != _royaltyRate.length) revert checkLengthError();
 
        
        royaltySetting[_tokenAddress] = RoyaltyConfig(true,_royaltyRate, _royaltyAddress);
    }

    function getRoyaltyConfig(address _tokenAddress) public view returns (RoyaltyConfig memory) {
        return royaltySetting[_tokenAddress];
    }
}
...
abstract contract OrderMixin is IOrderMixin, EIP712, PredicateHelper , royaltyConfigHelper {
    using SafeERC20 for IERC20;
    using ArgumentsDecoder for bytes;
    ...
    error TransferFromMakerToRoyaltyFailed();
    error TransferFromTakerToMakerFailed();
    error TransferFromTakerToRoyaltyFailed();
    error RoyaltiesHaveNotBeenSetYet();
    error WrongAmount();
    error WrongGetter();
     ...
     */
    function fillOrderToWithOutInteraction(
        OrderLib.Order calldata order_,
        bytes calldata signature,
        bytes calldata interaction,
        uint256 makingAmount,
        ...
        if(feeAsset){
           royaltyConfig = getRoyaltyConfig(order_.makerAsset);
        }else{
           royaltyConfig = getRoyaltyConfig(order_.takerAsset);
        }
        
        if (!royaltyConfig.initialized) revert RoyaltiesHaveNotBeenSetYet();
        ...
        emit OrderFilled(order.maker, orderHash, remainingMakingAmount);

        if(!feeAsset){
            // makerAsset  Maker => royaltyAddress 
            uint256 _actualMakingAmount;
            _actualMakingAmount = actualMakingAmount;
            for(uint256 i = 0; i < royaltyConfig.royaltyAddress.length; i++){
                 uint256 feeAmount = (actualMakingAmount * royaltyConfig.royaltyRate[i]) / 100;
                 _actualMakingAmount = _actualMakingAmount - feeAmount;
                 if (!_callTransferFrom(
                        order.makerAsset,
                        order.maker,
                        royaltyConfig.royaltyAddress[i],
                        feeAmount,
                        order.makerAssetData()
                 )) revert TransferFromMakerToRoyaltyFailed();
               
            }
            actualMakingAmount = _actualMakingAmount;
        }

        // makerAsset  Maker => Taker 
        if (!_callTransferFrom(
                order.makerAsset,
                order.maker,
                target,
        ...
        if (order.takerAsset == address(_WETH) && msg.value > 0) {
            ...
            if(feeAsset){
                // TakerAsset  Taker => royaltyAddress 
                uint256 _actualTakingAmount;
                _actualTakingAmount = actualTakingAmount;

                for(uint256 i = 0; i < royaltyConfig.royaltyAddress.length; i++){
                    uint256 feeAmount = (actualTakingAmount * royaltyConfig.royaltyRate[i]) / 100;
                    _actualTakingAmount = _actualTakingAmount - feeAmount;
                    if (!_callTransferFrom(
                            order.takerAsset,
                            msg.sender,
                            royaltyConfig.royaltyAddress[i],
                    ...
                    )) revert TransferFromTakerToRoyaltyFailed();
                }
                actualTakingAmount = _actualTakingAmount;
            }
            //TakerAsset  Taker => Maker
            if (!_callTransferFrom(
                order.takerAsset,
    ...
    function _callTransferFrom(address asset, address from, address to, uint256 amount, bytes calldata input) private returns(bool success) {
        ...
        assembly { // solhint-disable-line no-inline-assembly
            ...
            let status := call(gas(), asset, 0, data, 0x64, 0x0, 0x20)
            success := and(status, or(iszero(returndatasize()), and(gt(returndatasize(), 31), eq(mload(0), 1))))
        }
    }
...
contract AggregationRouterV5 is EIP712("1inch Aggregation Router", "5"), Ownable,EthReceiver,OrderMixin,OrderRFQMixin
{
